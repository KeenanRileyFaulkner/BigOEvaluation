...
// ANSWER: O(n), where n is the size of the array
...
// ANSWER: O(1) -> there is a bug, though. since the fuction returns void, it's not going to ever give the answer to the caller.

// Q: Given an array of numbers, what is the space complexity of this function?
// ANSWER: O(1)... numbers array is O(n) space complexity but is a param, not an intrinsic part of function. All that's stored in the function are a few vars
...
// ANSWER: No time complexity is better than O(1). However, it's not necessary to use the for loop here.
...
// ANSWER: O(1)... if i were not hard capped at 5, but were an arbitrarily large number, it would be O(n)
...
// ANSWER: No. It's possible to cut it down by 66% with the .map higher order array method, but it would still be O(n).
...
// ANSWER: They do the same thing, one just puts the keys into the letters object when the letters occur for the first time. The other predefines letters.
